...
def gen_layout():
    ...
    layout =html.Div([
                        html.Div([
                            html.Div([
                                html.Div("NHÂN VIÊN KHÔNG BẤM START TASK", style={'width':'100%','fontWeight': 'bold','background': '#FF938B'}),
                                html.A(
                                    children=[
                                        html.I(className="fas fa-file-download")
                                    ],
                                    id='download_user_withoutask',
                                    download="DETAIL_USERS_WITHOUT_TASK.csv",
                                    target="_blank",
                                    href = '',
...
def UPDATE_user_0_task(start_date,end_date,ddl_project,ddl_user,ddl_dept,user_opened,user_done,user_project,user_started,user_project_task,ac_project,ac_user,ac_log,ac_project_task,ac_workload,data_project, data_user,data_log,data_project_task,data_workload):
    ...
    df =DB_User.GET_USER(('user_0_task',start_date,end_date,ddl_project,ddl_user,ddl_dept,label,value,None,None,None,None))
    if df.empty:
        dff = pd.DataFrame({'u.account':['Không có ai'],'Thời gian':['Không']})
        return dff.to_dict(orient='records')
    else:
        return df.to_dict(orient='records')

@app_User.callback(
    Output('download_user_withoutask', 'href'),
    [Input('download_user_withoutask', 'n_clicks'),
     Input('user_0_task', 'data')]
)
def UPDATE_DOWNLOAD(click, data):
    df = pd.DataFrame.from_dict(data)

    csv_string = df.to_csv(index=False, encoding='utf-8', header=['Tên nhân viên','Thời gian'])
