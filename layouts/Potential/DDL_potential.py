...
def gen_ddl(id_ddl, placeholder, className):
    data = DB_potential.Get_ddl_potential((id_ddl))
    ...
    elif id_ddl in ['ddl_potential_mkt', 'ddl_potential_service', 'ddl_potential_status']:
        options = [{'value': data.values[i][0], 'label': data.values[i]
                    [0], 'title': data.values[i][0]} for i in range(data.shape[0])]
...
def gen_layout_row_1():
    layout = html.Div([
        gen_ddl('ddl_potential_company', "Chọn cơ hội",
                'col-sm-12 col-md-4 col-lg-4 mb-4'),
        gen_ddl('ddl_potential_sales_stage', "Chọn giai đoạn bán hàng",
                'col-sm-12 col-md-4 col-lg-4 mb-4'),
        # gen_ddl('ddl_potential_status', "Chọn trạng thái", 'col-sm-12 col-md-3 col-lg-3 mb-3'),
        gen_ddl('ddl_potential_product', "Chọn sản phẩm",
                'col-sm-12 col-md-4 col-lg-4 mb-4'),
...
def gen_layout_row_2():
    layout = html.Div([
        gen_ddl('ddl_potential_dept', "Chọn phòng ban",
                'col-sm-12 col-md-6 col-lg-6 mb-4'),
        gen_ddl('ddl_potential_assign', "Chọn nhân viên phân công",
                'col-sm-12 col-md-6 col-lg-6 mb-4'),
        # gen_ddl('ddl_potential_mkt', "Chọn nhân viên marketing", 'col-sm-12 col-md-3 col-lg-3 mb-3'),
        # gen_ddl('ddl_potential_service', "Chọn nhân viên giới thiệu", 'col-sm-12 col-md-3 col-lg-3 mb-3'),
    ], className="row", style={'flexWrap': 'wrap'})
    return layout
...
@app_Potential.callback(
    ...
    Input('num_potential_bysalestage', 'selectedData'),
    Input('detail_potential_delay', 'active_cell'),
    Input('detail_potential_delay_2', 'active_cell'),
    Input('detail_potential', 'active_cell'),
...
def update_ddl_source(sales_stage, ac_delay1, ac_delay2, ac_detail, sales_stage_options, data_delay1, data_delay2, data_detail):
    ...
    labels = {option["label"]: option["value"]
              for option in sales_stage_options}
...
@app_Potential.callback(
    ...
    Input('detail_potential_delay_2', 'active_cell'),
    Input('detail_potential', 'active_cell'),
    Input('comment_detail', 'active_cell'),
    Input('activity_detail', 'active_cell'),
    ...
    State('detail_potential_delay_2', 'data'),
    State('detail_potential', 'data'),
    State('comment_detail', 'data'),
    State('activity_detail', 'data'),
)
def update_ddl_user(ac_delay1, ac_delay2, ac_detail, ac_assigned_user1, ac_assigned_user2, ac_comment, ac_activity, user_assign_options, data_delay1, data_delay2, data_detail, data_comment, data_activity):
    ctx = dash.callback_context
    datatable = {'detail_potential_delay': data_delay1,
                 'detail_potential_delay_2': data_delay2,
                 'detail_potential': data_detail,
                 'comment_detail': data_comment,
                 'activity_detail': data_activity
                 }
    ...
    labels = {option["label"].split(
        ' (')[1][:-1]: option["value"] for option in user_assign_options}
...
def update_ddl_user(ac_detail, ac_service_user1, ac_service_user2, user_service_options, data_detail):
    ...
    labels = {option["label"]: option["value"]
              for option in user_service_options}
