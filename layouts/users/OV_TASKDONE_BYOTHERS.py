...
def gen_layout():
    ...
    title = 'SỐ LƯỢNG TASK ĐÃ TẠO HOÀN THÀNH BỞI NHÂN VIÊN KHÁC'
    color = '#1DDAB7'
    icon = 'ov_others.svg'
    layout = create_card(title,'ov-taskdone-byothers',color,icon)
           
    return layout

@app_User.callback(
    Output("ov-taskdone-byothers","children"),
...
def UPDATE_OV_TASKDONE(start_date,end_date,ddl_project,ddl_user,ddl_dept,user_opened,user_done,taskdone_status,user_project,user_started,user_project_task,ac_project,ac_user,ac_log,ac_project_task,ac_workload,data_project, data_user,data_log,data_project_task,data_workload):
    ...
    df = DB_User.GET_USER(('ov_taskdone_byothers',start_date,end_date,ddl_project,ddl_user,ddl_dept,label,value,None,None,None,None))
    return df['Số lượng task đã tạo hoàn thành bởi nhân viên khác'].values[0]
