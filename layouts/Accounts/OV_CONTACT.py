...
def gen_layout():
    ...
    color = '#01CBCF'
    icon = 'contact.svg'
    layout = create_card(title, 'ov_contact', color, icon)

    return layout


@app_Accounts.callback(
    Output("ov_contact", "children"),
    Output("ov_contact_diff", "children"),
...
def UPDATE_OV_CONTACT(date_value, ddl_timetype, start_date, end_date, ddl_leadsource, ddl_industry, ddl_account, ddl_user_assign, ddl_dept, status, industry, user_assign, user_marketing, user_service, map_city, ac_user_detail, ac_account_detail, ac_city, data_user_detail, data_account_detail, data_city):
    ...
    df = DB_Accounts.GET_ACCOUNTS(('ov_contact', ddl_timetype, start_date, end_date, ddl_leadsource, ddl_industry,
    ...
    if date_value is not None:
        ...
        df2 = DB_Accounts.GET_ACCOUNTS(('ov_contact', ddl_timetype, start_date_last, end_date_last, ddl_leadsource, ddl_industry,
                                       ddl_account, ddl_user_assign,  None, None,  ddl_dept, None, None, None, None, label, value))

        end_value = df['Tổng số lượng liên hệ'].values[0]
        start_value = df2['Tổng số lượng liên hệ'].values[0]
        ...
        return df['Tổng số lượng liên hệ'].values[0], '-'
