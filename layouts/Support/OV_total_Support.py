...
def gen_layout():
    ...
    layout = create_card(title, 'ov_total_potential', color, icon)
...
def update_ov_total_support(date_type, start_date, end_date, account, user, dept, status, pareto_user, bar_status, pie_pri, pie_location, ac_delay, ac_detail, data_delay, data_detail):
    ...
    print(label)
    values = DB_Support.get_Support(
        ('ov_support', date_type, start_date, end_date, account, None, user, dept, status, None, label, value))
