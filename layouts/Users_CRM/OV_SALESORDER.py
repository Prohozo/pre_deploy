...
def gen_layout():
    ...
    color = 'linear-gradient(135deg, rgba(238,174,202,1) 0%, rgba(148,187,233,1) 100%)'
    icon = 'sales.svg'
    layout = create_card(title,'ov_salesorder',color,icon)
           
    return layout

@app_UsersCRM.callback(
    Output("ov_salesorder","children"),
    Output("ov_salesorder_diff","children"),
...
def UPDATE_OV_SALESORDER(date_value,start_date,end_date,ddl_account,ddl_user_assign,ddl_dept,salesorder,task_created,task_finished,ac_withoutactivity,ac_task_delay,ac_activity_detail,ac_without_login,data_withoutactivity,data_task_delay,data_activity_detail,data_without_login):
    ...
    df = DB_Users_CRM.GET_USERSCRM(('ov_salesorder',start_date,end_date,ddl_account,ddl_user_assign,ddl_dept,label,value))
    
    if date_value is not None:
        start_date_last,end_date_last = last_time(date_value)
        df2 = DB_Users_CRM.GET_USERSCRM(('ov_salesorder',start_date_last,end_date_last,ddl_account,ddl_user_assign,ddl_dept,label,value))

        end_value = df['Tổng số lượng đơn hàng'].values[0]
        start_value  = df2['Tổng số lượng đơn hàng'].values[0]
        ...
        return df['Tổng số lượng đơn hàng'].values[0], '-'
