...
class Get_Params:
    ...
    def Get_Value(self, ctx, data=None):
        ...
        else: self._id_ = info['prop_id'].split('.')[0] 
        value = info['value']
        ...
        if self._id_ in cf.Bar_Chart.keys() and value is not None:
            if 'customdata' in value['points'][0]:
                value = value['points'][0]['customdata']
            else: value = value['points'][0]['label']
        ...
        if self._id_ in cf.Map.keys() and value is not None:
            value = value['points'][0]['location']
            return cf.Map[self._id_], value
