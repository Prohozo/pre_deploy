...
def gen_layout():
    layout = html.Div([
        html.Div([
            html.Div([
                html.B(html.Div("SỐ KHÁCH HÀNG TIỀM NĂNG THEO TRẠNG THÁI", style={
            ...
            dcc.Graph(id="num_lead_bystatus", style={
                      'width': '100%', 'height': '92%'}),
...
@app_Lead.callback(
    Output("num_lead_bystatus", "figure"),
...
def update_num_lead_bystatus(date_type, start_date, end_date, company, dept, source, status, assign, city, ac_industry, ac_user):
    ...
    df = DB_Lead.Get_Lead(('num_lead_bystatus', date_type, start_date, end_date, None, None, None,
                          None, assign, status, source, company, dept, city, label, value, None, None, None, None))

    if df.empty:
        df = pd.DataFrame({'ld.leadstatus': ['Không có giá trị'], 'num': [0]})

    return Graph_Lead.grh_BarChart(df['ld.leadstatus'], df['num'], label_x='<b>Trạng thái</b>', label_y='<b>Số tiềm năng</b>', marker_color=['#107a8b', '#107a8b'], margin=[50, 150, 100, 20])
