                        ...
                        html.Div([
                            html.Div([
                                html.A(
                                    ...
                                    id='download_detail_task_assign',
                                    download='TASK_DA_GIAO.csv',
                                    target="_blank",
                                    href='',
...
@app_Task.callback(
    Output("detail_task_assign", "data"),
    # Output("detail_task_assign", "columns"),
    Output('detail_task_assign', 'page_count'),
    Output('detail_task_assign', 'style_data_conditional'),
    ...
    Input('detail_task_assign', 'page_current'),
    Input('detail_task_assign', 'page_size'),
    Input('detail_task_assign', 'sort_by'),
    Input('detail_task_assign', 'filter_query'),
...
def UPDATE_detail_task(start_date, end_date, project_id, user, story, dept, ac_task_delay, ac_task_byproject, ac_task_bystory, ac_task_numpri, ac_dept, page_current, page_size, sort_by, filter, data_task_delay, data_task_byproject, data_task_bystory):
    ...
    df=DB_Task.Get_task(('detail_assign', start_date, end_date, project_id, user, story, dept, label, value, None, None,  None,None))

    # columns = [{"name": i, "id": i} if (i != 'Tên task' or i != 'Dự án') else {"name": i, "id": i, 'presentation': 'markdown'} for i in df.columns]
    ...
    dff['Ngày giao task'] = pd.DatetimeIndex(dff['Ngày giao task']).strftime("%d-%m-%Y %I:%M %p")
...
@app_Task.callback(
    Output('download_detail_task_assign', 'href'),
    Input('download_detail_task_assign', 'n_clicks'),
...
def UPDATE_DOWNLOAD(click, start_date, end_date, project_id, user, story, dept, ac_task_delay, ac_task_byproject, ac_task_bystory, ac_task_numpri, data_task_delay, data_task_byproject, data_task_bystory):
    ...
    if df.empty:
        csv_string = df.to_csv(index=False, encoding='utf-8', header=['Dự án', 'Trạng thái', 'Người được giao', 'Tên task', 'Loại', 'Ngày giao task', 'Ngày bắt đầu', 'Số giờ ước tính', 'Số giờ thực hiện','Deadline'])
    else:
        csv_string = df.to_csv(index=False, encoding='utf-8', header=['Dự án', 'Trạng thái', 'Người được giao', 'Tên task', 'Loại', 'Ngày giao task', 'Ngày bắt đầu', 'Số giờ ước tính', 'Số giờ thực hiện','Deadline'])
