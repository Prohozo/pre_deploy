...
def gen_layout():
    ...
    layout = html.Div([
                html.Div([
                    dcc.Graph(id="grh_bar_project_task", style={'width': '100%', 'height': '100%'})
...
@app_User.callback(
    Output("grh_bar_project_task","figure"),
...
def UPDATE_SL_PROJECT_TASK(start_date,end_date,ddl_project,ddl_user,ddl_dept,user_opened,user_done,user_project,user_started,user_project_task,ac_project,ac_user,ac_log,ac_project_task,ac_workload,data_project, data_user,data_log,data_project_task,data_workload):
    ...
    df = DB_User.GET_USER(('bar_project_task',start_date,end_date,ddl_project,ddl_user,ddl_dept,label,value,None,None,None,None))
    if df.empty:
        df = pd.DataFrame({'account_nv': ['Không có giá trị'], 'status_project': ['Không có giá trị'], 'num_project': [0]})

        
    return  Graph.grh_StackedBarChart(df,'account_nv','num_project',color='status_project',title='SỐ LƯỢNG DỰ ÁN NHÂN VIÊN ĐANG THỰC THI',label_x='Tên nhân viên',label_y='Số lượng dự án',legend_title='Trạng thái',color_discrete_sequence=['#FF938B','#FFD45D','#927EFA','#24E8A7','#98A8AE'],category_orders={"status_project":["closed","doing","wait","suspended"]},margin = [50, 150, 35, 35])
