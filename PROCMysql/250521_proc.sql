...
BEGIN
	...
	SET QUERY1 = CONCAT(QUERY1, 'AND DATEDIFF(DATE(NOW()), DATE(t.lastEditedDate)) >= 7 AND t.STATUS IN (''doing'', ''wait'', ''pause'')AND NOT (t.deadline = ''0000-00-00'' AND t.status=''wait'') AND t.deleted <> ''1'' AND p.deleted <> ''1''');
...
BEGIN
	...
	SET myquery1 = CONCAT ('SELECT account 
									FROM zt_user 
									LEFT JOIN zt_dept d
									ON dept = d.id
									WHERE deleted =''0'' AND ACCOUNT NOT IN (',myquery,')');	
									
	IF (dept_id IS NOT NULL) THEN
		SET myquery1  = CONCAT(myquery1,' AND dept = ''', dept_id, '''');
	END IF;
	SET myquery1 = CONCAT(myquery1,' ORDER BY account');
