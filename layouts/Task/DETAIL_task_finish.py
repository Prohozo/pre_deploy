                        ...
                        html.Div([
                            html.Div([
                                html.A(
                                    ...
                                    id='download_detail_task_finish',
                                    download='TASK_DA_HOAN_THANH.csv',
                                    target="_blank",
                                    href='',
...
@app_Task.callback(
    Output("detail_task_finish", "data"),
    Output('detail_task_finish', 'page_count'),
    Output('detail_task_finish','style_data_conditional'),
    ...
    Input('detail_task_finish', 'page_current'),
    Input('detail_task_finish', 'page_size'),
    Input('detail_task_finish', 'sort_by'),
    Input('detail_task_finish', 'filter_query'),
...
def UPDATE_detail_task(start_date, end_date, project_id, user, story, dept, ac_task_delay, ac_task_byproject, ac_task_bystory, ac_task_numpri, ac_dept, page_current, page_size, sort_by, filter, data_task_delay, data_task_byproject, data_task_bystory):
    ...
    df = DB_Task.Get_task(('detail_finish', start_date, end_date, project_id, user, story, dept, label, value, None, None, None, None))
    ...
    dff['Ngày hoàn thành'] = pd.DatetimeIndex(dff['Ngày hoàn thành']).strftime("%d-%m-%Y %I:%M %p")
...
@app_Task.callback(
    Output('download_detail_task_finish', 'href'),
    Input('download_detail_task_finish', 'n_clicks'),
...
def UPDATE_DOWNLOAD(click, start_date, end_date, project_id, user, story, dept, ac_task_delay, ac_task_byproject, ac_task_bystory, ac_task_numpri, data_task_delay, data_task_byproject, data_task_bystory):
    ...
    if df.empty:
        csv_string = df.to_csv(index=False, encoding='utf-8', header=['Dự án', 'Trạng thái', 'Người hoàn thành','Tên task', 'Ngày hoàn thành', 'Ngày bắt đầu', 'Số giờ ước tính', 'Số giờ thực hiện', 'Deadline'])
    else:
        csv_string = df.to_csv(index=False, encoding='utf-8', header=['Dự án', 'Trạng thái', 'Người hoàn thành','Tên task', 'Ngày hoàn thành', 'Ngày bắt đầu', 'Số giờ ước tính', 'Số giờ thực hiện', 'Deadline'])
