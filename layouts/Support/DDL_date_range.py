...
def gen_layout():
    start_date_plh = date(2016, 9, 7).strftime('%d/%m/%Y')
    ...
    layout = html.Div([
        ...
        dcc.DatePickerRange(
            ...
            min_date_allowed=date(2016, 9, 7),
...
@app_Support.callback(
    ...
    Input('support_by_time_date', 'clickData'),
    Input('support_by_time_month', 'clickData'),
    Input('support_by_time_quarter', 'clickData'),
    Input('support_by_time_year', 'clickData'),
)
def update_date_filter(date_value, support_date, support_month, support_quarter, support_year):
    ctx = dash.callback_context
    label, value = GParams_Support.Get_Value(ctx)

    # Thay doi dropdown list (Khoang thoi gian)
    if date_value is not None and label not in ('date', 'month', 'quarter', 'year'):
        if date_value == 0:
