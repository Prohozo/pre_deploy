...
@app_Lead.callback(
    ...
    Input('lead_by_time_date', 'clickData'),
    Input('lead_by_time_month', 'clickData'),
    Input('lead_by_time_quarter', 'clickData'),
    Input('lead_by_time_year', 'clickData'),
)
def update_date_filter(date_value, ac_date, ac_month, ac_quarter, ac_year):
    ctx = dash.callback_context
    label, value = GParams_Lead.Get_Value(ctx)

    # Thay doi dropdown list (Khoang thoi gian)
    if date_value is not None and label not in ('date', 'month', 'year', 'quarter'):
        if date_value == 0:
