...
def gen_layout():
    ...
    color = 'linear-gradient(50deg, rgba(62,210,140,1) 0%, rgba(19,174,177,1) 100%)'
    icon = 'ov_created.svg'
    layout = create_card(title,'ov_task_created',color,icon)
           
    return layout

@app_UsersCRM.callback(
    Output("ov_task_created","children"),
    Output("ov_task_created_diff","children"),
...
def UPDATE_OV_TASK_CREATED(date_value,start_date,end_date,ddl_account,ddl_user_assign,ddl_dept,salesorder,task_created,task_finished,ac_withoutactivity,ac_task_delay,ac_activity_detail,ac_without_login,data_withoutactivity,data_task_delay,data_activity_detail,data_without_login):
    ...
    df = DB_Users_CRM.GET_USERSCRM(('ov_task_created',start_date,end_date,ddl_account,ddl_user_assign,ddl_dept,label,value))
    
    if date_value is not None:
        start_date_last,end_date_last = last_time(date_value)
        df2 = DB_Users_CRM.GET_USERSCRM(('ov_task_created',start_date_last,end_date_last,ddl_account,ddl_user_assign,ddl_dept,label,value))
