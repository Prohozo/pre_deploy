...
def gen_layout():
    ...
    layout = html.Div([
                html.Div([
                    dcc.Graph(id="grh_bar_project", style={'width': '100%', 'height': '100%'})
...
def UPDATE_SL_PROJECT(start_date,end_date,ddl_project,ddl_user,ddl_dept,user_opened,user_done,user_project,user_started,user_project_task,ac_project,ac_user,ac_log,ac_project_task,ac_workload,data_project, data_user,data_log,data_project_task,data_workload):
    ...
    df = DB_User.GET_USER(('bar_project',start_date,end_date,ddl_project,ddl_user,ddl_dept,label,value,None,None,None,None))
   
    return  Graph.grh_BarChart(df['account_nv'],df['Số lượng project'],marker_color=['#FFD45D','#FFD45D'],title='SỐ LƯỢNG PROJECT THEO NHÂN VIÊN',margin = [50, 170, 35, 35])
