...
def gen_layout():
    ...
    layout =  html.Div([
                        html.Div([
                            html.Div([
                                html.Div("SỐ LƯỢNG DỰ ÁN THEO TỪNG BỘ PHẬN", style={'width':'100%','background': '#60B664'}),
                                html.A(
                                    children=[
                                        html.I(className="fas fa-download")
                                    ],
                                    id='download_duan_dept',
                                    download='duan_dept.csv',
                                    target="_blank",
                                    href = '',
                    ...
                    ], className="col-lg-4 m-b-15 graph_kt")

    return layout


@app_DuAn.callback(
    [Output("SoDuAn_dept", "data"),
     Output("SoDuAn_dept", "style_data_conditional")],
...
def update_DuAn_dept_bang(name, dept, start_act,end_act, TL_DA_dept, status_DA, action_TG_month, action_TG_day, action_TG_year, AC_chitiet_tt, AC_chitiet, AC_td, DT_chitiet_tt, DT_chitiet, DT_td):
    ctx = dash.callback_context
    data_in_dt = {'detail_chitiet_trangthai': DT_chitiet_tt,
    ...
    df = DB_DuAn.GET_DuAn_dept_bang((name, dept, start_act,end_act, label, value))
    sdc =   (
                [{
                    'if': {'row_index': 'even'},
...
@app_DuAn.callback(
     Output('download_duan_dept', 'href'),
    [Input('download_duan_dept','n_clicks'),
     Input('SoDuAn_dept','data')]
)
...
def UPDATE_DOWNLOAD(click,data):
    ...
    if df.shape[0] == 0:
        return '#'
    csv_string = df.to_csv(index=False, encoding='utf-8',header=['Tên bộ phận','Số lượng dự án'])
