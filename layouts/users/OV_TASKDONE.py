...
def gen_layout():
    ...
    title = 'TỔNG SỐ LƯỢNG TASK HOÀN THÀNH'
    color = '#1DDAB7'
    icon = 'ov_taskdone1.svg'
    layout = create_card(title,'ov-taskdone',color,icon)
           
    return layout

@app_User.callback(
    Output("ov-taskdone","children"),
    Output("ov-taskdone_diff","children"),
...
def UPDATE_OV_TASKDONE(date_value,start_date,end_date,ddl_project,ddl_user,ddl_dept,user_opened,user_done,taskdone_status,user_project,user_started,user_project_task,ac_project,ac_user,ac_log,ac_project_task,ac_workload,data_project, data_user,data_log,data_project_task,data_workload):
    ...
    df = DB_User.GET_USER(('ov_taskdone',start_date,end_date,ddl_project,ddl_user,ddl_dept,label,value,None,None,None,None))
    
    if date_value is not None:
        start_date_last,end_date_last = last_time(date_value)
        df2 = DB_User.GET_USER(('ov_taskdone',start_date_last,end_date_last,ddl_project,ddl_user,ddl_dept,label,value,None,None,None,None))
