...
def gen_layout():
    layout = html.Div([
        html.Div([
            html.Div([
                html.B(html.Div("HỖ TRỢ THEO ĐỊA ĐIỂM", style={
            ...
            dcc.Graph(id="num_Support_bylocation", style={
                      'width': '100%', 'height': '92%'}),
...
@app_Support.callback(
    Output("num_Support_bylocation", "figure"),
...
def update_num_Support_bylocation(date_type, start_date, end_date, account, account_status, user, dept, status, pareto_user, bar_status, pie_pri, pie_location, ac_delay, ac_detail, data_delay, data_detail):
    ...
    df = DB_Support.get_Support(('location', date_type, start_date, end_date,
                                account, account_status, user, dept, status, None, label, value))

    return Graph_Support.grh_DonutChart(df['num'], df['cf_893'], margin=[50, 10, 35, 35], colors=['#107a8b', '#2cb978', '#60B664', '#3b5441'])


@app_Support.callback(
    Output('num_Support_bylocation', 'clickData'),
