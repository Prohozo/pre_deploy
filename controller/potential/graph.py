...
class gen_graph:
    ...
    def grh_BarChart_percent(self, x, y, text=None,label_x = None, label_y = None,label_color = None, marker_color=[None,None], width = None , orientation='v', title=None, size_title=13, margin=[5, 5, 5, 5], customdata=None):
        '''Create bar chart'''
        l = len(x)
        ...
        data = [
            go.Bar(
                ...
                hovertemplate="%{x}: <br>%{y} <br>Tổng:%{customdata}" if orientation == 'v' else "%{x}",
                orientation = orientation,
                marker_color = self.maker_color(marker_color[0],marker_color[1],y),
                name='',
                cliponaxis=False,
    ...
    def grh_LineChart(self, x, y, mode, text=None, title=None, size_title=15, margin = [5, 5, 5, 5],color=None, height=None):
        data = [
            go.Scatter(
                x = x,
                y = y,
    ...
    def gen_Bar(self, x, y, text=None, orientation='v', textposition='auto', marker_color = [None,None], name='', customdata=None):
        l = len(x)
        GB = go.Bar(
                x = x, 
                y = y,
    ...
    def grh_GroupBarChart(self, data, x, y, name, font_size=15, barmode = 'group',margin = [5, 5, 5, 5], orientation = 'v',categoryarray=None,title=None):
        ...
        # data = [self.gen_Bar(x, y, name=name,orientation=orientation) for name in name]
        data = [self.gen_Bar([row[x]], [row[y]], name = row[name], orientation=orientation) for index, row in data.iterrows()]
        # print(data)
        # print([f'self.gen_Bar({row["ld.leadsource"]}, {row["ld.leadstatus"]}, name={row["num"]})' for index, row in data.iterrows()])
        # data.extend([self.gen_Bar(x2, y2, name=name,orientation=orientation) for name in name_2])
    ...
    def grh_StackedBarChart(self, data, x, y, color=None, label_x= None, label_y = None, legend_title = None, title=None, size_title=13, color_discrete_map=None, hovertemplate=None, margin = [5, 5, 5, 5]):
        fig = px.bar(data, x=x, y=y, color=color, color_discrete_map = color_discrete_map)
    ...
    def grh_StackedbarChart_x(self, data, x, y, name, font_size=15, barmode = 'stack',margin = [5, 5, 5, 5], orientation = 'v',categoryarray=None,title=None):
        data = [self.gen_Bar([row[x]], [row[y]], name = row[name], orientation=orientation, customdata=[row[name]], text=[row[y]]) for index, row in data.iterrows()]
                
        layout_bar = self.layout.copy()
        layout_bar.update({'margin':{'t': margin[0],'b':margin[1],'l':margin[2],'r':margin[3]}, 'barmode':barmode,'xaxis': {'categoryorder':'array', 'categoryarray':categoryarray}, 'legendgroup':"group"})
        layout_bar.update({
            'title': {
                'text':title,
                'font': {
    ...
    def grh_ParetoChart(self, x1, y1, x2, y2, text=None,label_x = None, label_y = None,label_color = None, marker_color=[None,None],color_line=None,mode=None, width = None , orientation='v', title=None, size_title=13, margin=[5, 5, 5, 5]):
        '''Create Pareto chart'''
        l = len(x1)
        ...
        layout_bar.update(yaxis2=dict(title='<b>Phần trăm cộng dồn<b>',
                                   overlaying='y',
                                   side='right',
                                   ticksuffix = '%'))
        layout_bar.update(hovermode='x unified',legend=dict(orientation='h',yanchor='bottom',xanchor='right',y=1.02,x=1.08))
