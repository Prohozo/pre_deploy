...
def gen_layout():
    ...
    layout = html.Div([
                html.Div([
                    dcc.Graph(id="grh_bar_task_created", style={'width': '100%', 'height': '100%'})
...
@app_UsersCRM.callback(
    Output("grh_bar_task_created","figure"),
...
def UPDATE_BAR_TASK_CREATED(start_date,end_date,ddl_account,ddl_user_assign,ddl_dept,salesorder,task_created,task_finished,ac_withoutactivity,ac_task_delay,ac_activity_detail,ac_without_login,data_withoutactivity,data_task_delay,data_activity_detail,data_without_login):
    ...
    df = DB_Users_CRM.GET_USERSCRM(('user_task_created',start_date,end_date,ddl_account,ddl_user_assign,ddl_dept,label,value))
   
    return  Graph_UsersCRM.grh_BarChart(df['Nhân viên'],df['Số lượng task'],label_x='NHÂN VIÊN',label_y='SỐ LƯỢNG TASK ĐÃ TẠO',marker_color=['#70B190','#70B190'],title='SỐ LƯỢNG TASK ĐÃ TẠO THEO NHÂN VIÊN',margin = [50, 150, 60, 35])
