...
def gen_layout():
    ...
    title = 'Tổng cơ hội đang sale'
    color = 'linear-gradient(to right, rgb(34, 193, 195), rgb(253, 187, 45))'
    icon = 'sale.svg'

    layout = create_card(title, 'ov_total_sale', color, icon)

    return layout


@app_Potential.callback(
    Output("ov_total_sale", "children"),
...
def update_ov_total_potential_sale(date_type, start_date, end_date, potential, sales_stage, product, dept, assign):
    ctx = dash.callback_context
    datatable = {}

    label, value = GParams_Potential.Get_Value(ctx, datatable)

    values = DB_potential.Get_potential(('total_potential_sale', date_type, start_date, end_date, None, None, None, None,
    ...
    return f'{int(values.total_potential_sale)} ({round(float(values.total_potential_sale/total.total_potential*100),2)}%)'
