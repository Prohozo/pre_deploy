...
def gen_layout():
    ...
    title = 'DỰ ÁN WAIT'
    color = '#60B664'
    layout = html.Div([create_card(title,_id_,color,'ov-kt ov-5 col-lg-12 col-md-12 col-xs-12')], className='row')
    return layout

@app_DuAn.callback(
    Output("DuAn_wait", "children"),
...
def update_overview_DuAN(name, dept, start_act,end_act, TL_DA_dept, status_DA, action_TG_month, action_TG_day, action_TG_year, AC_chitiet, AC_DA_dept, AC_chitiet_tt, AC_td,  DT_chitiet, DT_DA_dept, DT_chitiet_tt, DT_td):
    ...
    df = DB_DuAn.GET_overview_DuAN_wait((name, dept, start_act,end_act, label, value))
