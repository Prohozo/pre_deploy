...
-- Dumping structure for procedure fpms.dsa_sp_action_multiline
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `dsa_sp_action_multiline`(
...
BEGIN
	...
	SET date_ = CASE WHEN time_type = 'D' THEN 'CAST(a.date AS DATE)'
						  WHEN time_type = 'M' THEN CONCAT('CAST(CONCAT(CAST(YEAR(a.date) AS NCHAR), ''-'', CASE WHEN MONTH(a.date) < 10 THEN CONCAT( ''0'',CAST(MONTH(a.date) AS NCHAR)) ELSE CAST(MONTH(a.date) AS NCHAR) END,''-01'') AS DATE)')
						  ELSE 'CAST(CONCAT(CAST(YEAR(a.date) AS NCHAR),''/01/01'') AS DATE)' END;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `dsa_sp_detail_DuAn_Bang`(
	IN `name_` NVARCHAR(200),
	IN `dept_` NVARCHAR(20),
	IN `start_act` NVARCHAR(20),
	IN `end_act` NVARCHAR(20),
	IN `col_` NVARCHAR(20),
	IN `values_` NVARCHAR(100)
)
BEGIN
	...
	DECLARE query_date nvarchar(300);
	
	SET query_action = CASE WHEN col_ = 'action' THEN CONCAT(' AND a.action = ''', values_, '''') ELSE ' AND a.action IN ( ''opened'', ''closed'', ''started'' , ''suspended'')' END;
	SET query_date = CASE WHEN start_act IS NOT NULL THEN CONCAT(' AND a.date between ''', start_act, ''' AND ''', end_act, '''') ELSE '' END; 
	...
	SET query_ = CONCAT('SELECT c.name, DATEDIFF( CURDATE(), MAX(a.date)) tt_date',
				   			 ...
				   			 ' WHEN c.status = ''wait'' then c.status', 
								 ...
								 ' LEFT JOIN zt_user d ON a.actor = d.account',
								 ' LEFT JOIN zt_dept e ON d.dept = e.id',
								 ' WHERE c.id IN (SELECT DISTINCT(project) FROM zt_action a WHERE objectType = ''project''', query_action, query_date,') 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `dsa_sp_overview_DuAn`(
	IN `name_` NVARCHAR(200),
	IN `dept_` NVARCHAR(20),
	IN `start_act` NVARCHAR(10),
	IN `end_act` NVARCHAR(10),
	IN `col_` NVARCHAR(20),
	IN `values_` NVARCHAR(100)
)
BEGIN

	DECLARE query_ nvarchar(3000);
	DECLARE query_lh nvarchar(200);
	DECLARE query_action nvarchar(300);
	DECLARE query_date nvarchar(300);
	
	SET query_action = CASE WHEN col_ = 'action' THEN CONCAT(' AND e.action = ''', values_, '''') ELSE ' AND e.action IN ( ''opened'', ''closed'', ''started'' , ''suspended'')' END;
	SET query_date = CASE WHEN start_act IS NOT NULL THEN CONCAT(' AND e.date between ''', start_act, ''' AND ''', end_act, '''') ELSE '' END; 
	...
	SET query_ = CONCAT(' SELECT  COUNT(DISTINCT d.id) SoDA
								...
								WHERE d.id IN (SELECT DISTINCT(project) FROM zt_action e WHERE objectType = ''project''', query_action, query_date,') AND d.deleted = ''0''');
	/* Lọc theo dự án */
	IF (name_ IS NOT NULL) Then
...
BEGIN
	...
	SET query_ = CONCAT(' SELECT COUNT(DISTINCT d.id) SoDA
								...
								LEFT JOIN zt_user b ON a.finishedBy = b.account 
								LEFT JOIN zt_dept c ON b.dept = c.id
								WHERE d.id IN (SELECT DISTINCT(project) FROM zt_action e WHERE objectType = ''project''', query_action, query_date,') AND d.status = ''doing''AND d.deleted = ''0''');
	/* Lọc theo dự án */
	IF (name_ IS NOT NULL) Then
        SET query_ = CONCAT(query_ , ' AND d.id = ''', name_, '''');
   END IF;
...
BEGIN
	...
	SET query_ = CONCAT(' SELECT COUNT(DISTINCT d.id) SoDA
								...
								LEFT JOIN zt_user b ON a.finishedBy = b.account 
								LEFT JOIN zt_dept c ON b.dept = c.id
								WHERE d.id IN (SELECT DISTINCT(project) FROM zt_action e WHERE objectType = ''project''', query_action, query_date,') AND d.status = ''suspended''AND d.deleted = ''0''');
...
BEGIN
	...
	SET query_ = CONCAT(' SELECT COUNT(DISTINCT d.id) SoDA
								...
								LEFT JOIN zt_user b ON a.finishedBy = b.account 
								LEFT JOIN zt_dept c ON b.dept = c.id
								WHERE d.id IN (SELECT DISTINCT(project) FROM zt_action e WHERE objectType = ''project''', query_action, query_date,') AND d.status = ''wait''AND d.deleted = ''0''');
	/* Lọc theo dự án */
	IF (name_ IS NOT NULL) Then
        SET query_ = CONCAT(query_ , ' AND d.id = ''', name_, '''');
   END IF;
...
BEGIN
	...
	SET query_ = CONCAT(' SELECT c.name, COUNT(DISTINCT a.project) SoDA
								...
								WHERE a.id IN (SELECT DISTINCT(project) FROM zt_action e WHERE objectType = ''project''', query_action, query_date,') AND a.deleted = ''0'' AND b.deleted = ''0''');
...
BEGIN
	...
	SET query_ = CONCAT('SELECT CASE WHEN a.status = ''suspended'' then a.status', 
								...
								' WHERE a.id IN (SELECT DISTINCT(project) FROM zt_action e WHERE objectType = ''project''', query_action, query_date,') AND a.deleted = ''0''');
...
BEGIN
	...
	DECLARE query_ nvarchar(4000);
	DECLARE query_lh nvarchar(400);
	DECLARE query_action nvarchar(400);
	DECLARE query_date nvarchar(300);
...
BEGIN 
	...
	SET query_ = CONCAT(' SELECT d.name, SUM(CASE WHEN a.STATUS != ''closed'' THEN a.left ELSE 0 END) AS left_, SUM(a.consumed) COST, 
		 					  CONCAT(CAST(CAST(SUM(a.consumed) /( SUM(a.consumed)+SUM(CASE WHEN a.STATUS != ''closed'' THEN a.left ELSE 0 END))*100 AS DECIMAL(10,2)) AS NCHAR), ''%'') txt_tile_ht',
						' FROM zt_project d LEFT JOIN zt_task a ON d.id = a.project',
						' LEFT JOIN zt_user b ON a.openedBy = b.account',  
						' LEFT JOIN zt_dept c ON b.dept = c.id',
						' WHERE d.id IN (SELECT DISTINCT(project) FROM zt_action e WHERE objectType = ''project''', query_action, query_date,') AND a.deleted = ''0'' AND d.deleted = ''0'' AND a.parent <> -1');
...
BEGIN
	...
	DECLARE query_date nvarchar(300);
		...
		SET query_ = CONCAT(' SELECT c.name, COUNT(DISTINCT a.project)/(SELECT COUNT(*) FROM zt_project WHERE deleted = ''0'') * 100 AS TiLe,
									...
									LEFT JOIN zt_dept c ON b.dept = c.id
									LEFT JOIN zt_project d ON a.project = d.id 
									WHERE d.id IN (SELECT DISTINCT(project) FROM zt_action e WHERE objectType = ''project''', query_action, query_date,') AND d.deleted = ''0''  AND b.deleted = ''0''');
...
BEGIN
	...
	IF id_proc = 'dept_order' THEN
		CALL `dsa_task_dept_order` (start_date,end_date,project_id,username, story, dept,col,val);
	END IF;
	
	IF id_proc = 'dept' THEN
		CALL `dsa_task_dept` (start_date,end_date,project_id,username, story, dept,col,val);
	END IF;
	
	IF id_proc = 'bar_taskopened' THEN
		CALL `dsa_task_taskopened_bar` (start_date,end_date,project_id,username,dept,col,val);
	END IF;
	
	IF id_proc = 'bar_taskdone' THEN
		CALL `dsa_task_taskdone_bar` (start_date,end_date,project_id,username,dept,col,val);
	END IF;
	...
	IF id_proc = 'num_hour_bytime' THEN
		CALL `dsa_task_num_hour_bytime`(start_date, end_date, project_id, username, story, dept, col, val, year_, month_, quarter_, day_);
	END IF;
	
	IF id_proc = 'num_est_hour_bytime' THEN
		CALL `dsa_task_num_est_hour_bytime`(start_date, end_date, project_id, username, story, dept, col, val, year_, month_, quarter_, day_);
	END IF;
	...
	IF id_proc = 'num_task_bytime' THEN
		CALL `dsa_task_num_task_bytime` (start_date, end_date, project_id, username, story, dept, col, val, year_, month_, quarter_, day_);
	END IF;
...
BEGIN
	...
	SET QUERY1 = 
	'SELECT
		CONCAT(''['',p.name,'']'',''(http://project.fastdn.com.vn/index.php?m=project&f=task&project='',p.id,'')'')  as ''Dự án'',
		t.status as ''Trạng thái'',
		t.assignedTo as ''Người được giao'',
		CONCAT(''['',t.name,'']'',''(http://project.fastdn.com.vn/index.php?m=task&f=view&taskID='',t.id,'')'')  as ''Tên task'',
		t.type as ''Loại'',
...
BEGIN
	...
	'SELECT
		p.name as ''p.name'',
		CONCAT(''['',t.name,'']'',''(http://project.fastdn.com.vn/index.php?m=task&f=view&taskID='',t.id,'')'')  as  ''t.name'',
...
-- Dumping structure for procedure fpms.dsa_task_dept
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `dsa_task_dept`(
	IN `start_date` DATE,
	IN `end_date` DATE,
	IN `project_id` INT,
	IN `username` VARCHAR(50),
...
BEGIN
	...
	LEFT JOIN
		zt_user u
	ON
		u.account = t.finishedBy
	LEFT JOIN
		zt_dept d
	ON
		d.id = u.dept
	WHERE t.deleted <> ''1'' AND p.deleted <> ''1'' AND t.parent <> -1 AND (t.status IN (''done'', ''closed'')) AND u.deleted <> ''1'' AND d.name IS NOT NULL AND  1=1 ';
	...
	CASE
		-- col, val binh thuong
		WHEN col IS NOT NULL AND col <> 's.title' AND col <> 'p.name' AND col <> 't.pri' THEN 
			SET QUERY1 = CONCAT(QUERY1,' AND (', col, ' LIKE N\'%', val ,'%\')');
			SET QUERY2 = CONCAT(QUERY2,' AND (', col, ' LIKE N\'%', val ,'%\')');
		
		-- Loc theo du an
		WHEN col IS NOT NULL AND col = 'p.name' THEN 
		...
		-- Loc theo story
		WHEN col IS NOT NULL AND col = 's.title' THEN 
		...
		-- Loc theo uu tien
		WHEN col IS NOT NULL AND col = 't.pri' THEN 
	...
	SET QUERY1 = CONCAT(QUERY1, ' GROUP BY 1,3) ');
	SET QUERY2 = CONCAT(QUERY2, ' GROUP BY 1,3) ');
	SET QUERY_ = CONCAT(QUERY1,' UNION ALL ', QUERY2);
	SET QUERY_ = CONCAT(QUERY_,'  ORDER BY 3, 1, 2 DESC');
...
-- Dumping structure for procedure fpms.dsa_task_dept_order
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `dsa_task_dept_order`(
	IN `start_date` DATE,
	IN `end_date` DATE,
	IN `project_id` INT,
	IN `username` VARCHAR(50),
...
BEGIN
	...
	SET QUERY2 = CONCAT(QUERY2, ' GROUP BY 1,3) ');
	SET QUERY_ = CONCAT(QUERY1,' UNION ALL ', QUERY2);
	SET QUERY_ = CONCAT(QUERY_,'  ORDER BY 2 DESC');
...
BEGIN
	...
	SET QUERY1_1 = 
		'(SELECT
			CONCAT(Year(t.finishedDate), '' - Q'', Quarter(t.finishedDate)) AS ''date_'',
			ROUND(SUM(t.estimate)) value,
			''Ước tính'' type
		FROM
			zt_task t
	...
	SET QUERY1 = CONCAT(QUERY1,' ORDER BY CAST(LEFT(date_,4) AS UNSIGNED), CAST(RIGHT(date_,1) AS UNSIGNED) ');
...
-- Dumping structure for procedure fpms.dsa_task_num_hour_bytime
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `dsa_task_num_hour_bytime`(
	IN `start_date` DATE,
	IN `end_date` DATE,
	IN `project_id` INT,
	IN `username` VARCHAR(50),
...
BEGIN
	...
	SET QUERY1 = 
		'SELECT
			CONCAT(Year(t.finishedDate), '' - Q'', Quarter(t.finishedDate)) AS ''time'',
			SUM(ROUND(t.consumed)) num
		FROM
			zt_task t
		INNER JOIN
			zt_project p
	...
	-- Gộp theo ngày và sắp xếp theo ngày từ h về trước
	SET QUERY1 = CONCAT(QUERY1,'GROUP BY 1 ORDER BY Year(t.finishedDate), QUARTER(t.finishedDate)');
...
BEGIN
	...
	-- Có filter username, k filter date
	IF username IS NOT NULL AND (start_date IS NULL AND end_date IS NULL) THEN
		SET QUERY1 = CONCAT(QUERY1,
			'SELECT
				p.name as ''p.name'',
				COUNT(CASE 
					WHEN 
						(t.openedDate <> ''0000-00-00 00:00:00'') 
						AND (t.assignedTo = \'', username ,'\' )  THEN t.id 
					ELSE 
						NULL 
					END) num_task_open,
				...
				COUNT(CASE 
					WHEN 
						(t.openedDate <> ''0000-00-00 00:00:00'') 
						AND (t.finishedDate = ''0000-00-00 00:00:00'') 
				...
				COUNT(CASE 
					WHEN 
						...
						AND (t.finishedDate = ''0000-00-00 00:00:00'') 
						AND (t.status = ''closed'' 
						AND t.closedDate <> ''0000-00-00 00:00:00'' 
						AND t.finishedDate = ''0000-00-00 00:00:00'' 
						AND t.canceledDate = ''0000-00-00 00:00:00'')
						THEN 
							t.id 
					ELSE 
						NULL 
	...
	-- Có filter date, k filter username
	ELSEIF username IS NULL AND (start_date IS NOT NULL AND end_date IS NOT NULL) THEN
		SET QUERY1 = CONCAT(QUERY1,
			'SELECT
				p.name as ''p.name'',
				...
				COUNT(CASE 
					WHEN 
						(t.openedDate <> ''0000-00-00 00:00:00'') 
						AND (t.openedDate BETWEEN \' ',start_date,' 00:00:00 \' ',' AND \' ', end_date, ' 23:59:59\' )  
						THEN t.id 
					ELSE 
						NULL 
					END) num_task_open,
				...
				COUNT(CASE 
					WHEN 
						(t.openedDate <> ''0000-00-00 00:00:00'') 
						AND (t.finishedDate = ''0000-00-00 00:00:00'') 
						AND (t.assignedDate BETWEEN \' ',start_date,' 00:00:00 \' ',' AND \' ', end_date, ' 23:59:59\' ) 
						AND (t.status NOT IN (''closed'',''done'')) 
						THEN 
							t.id 
					ELSE 
						NULL END) num_task_doing,
					
				COUNT(CASE 
					WHEN 
						(t.openedDate <> ''0000-00-00 00:00:00'') 
						AND (t.finishedDate BETWEEN \' ',start_date,' 00:00:00 \' ',' AND \' ', end_date, ' 23:59:59\' ) 
						AND (t.finishedDate <> ''0000-00-00 00:00:00'') 
						AND (t.status IN (''closed'',''done'')) 
						THEN 
							t.id 
				...
				COUNT(CASE 
					WHEN 
						...
						AND (t.assignedDate BETWEEN \' ',start_date,' 00:00:00 \' ',' AND \' ', end_date, ' 23:59:59\' ) 
						AND (t.finishedDate = ''0000-00-00 00:00:00'') 
						AND (t.status = ''closed'' AND t.canceledDate <> ''0000-00-00 00:00:00'') 
					THEN 
						t.id 
					ELSE 
						NULL 
	...
	-- Có filter date, filter username
	ELSEIF username IS NOT NULL AND (start_date IS NOT NULL AND end_date IS NOT NULL) THEN
		SET QUERY1 = CONCAT(QUERY1,
			'SELECT
				p.name as ''p.name'',
				...
				COUNT(CASE 
					WHEN (t.openedDate <> ''0000-00-00 00:00:00'') 
					AND (t.assignedTo = \'', username ,'\' ) 
					AND (t.assignedDate BETWEEN \' ',start_date,' 00:00:00 \' ',' AND \' ', end_date, ' 23:59:59\' ) 
					AND (t.finishedDate = ''0000-00-00 00:00:00'') 
					AND (t.status = ''closed'' 
	...
	-- Không filter date, filter username
	ELSE
		SET QUERY1 = CONCAT(QUERY1,
			'SELECT
				p.name as ''p.name'',
...
BEGIN
	...
	SET QUERY1 = 
		'SELECT
			CONCAT(Year(t.openedDate), '' - Q'', Quarter(t.openedDate)) AS ''t.openedDate'',
			COUNT(t.id) num
		FROM
			zt_task t
		INNER JOIN
	...
	-- Gộp theo ngày và sắp xếp theo ngày từ h về trước
	SET QUERY1 = CONCAT(QUERY1,'GROUP BY 1 ORDER BY Year(t.openedDate), QUARTER(t.openedDate)');
	SET QUERY2 = CONCAT(QUERY2,'GROUP BY 1 ORDER BY 1');
...
-- Dumping structure for procedure fpms.dsa_task_taskdone_bar
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `dsa_task_taskdone_bar`(
	IN `start_date` DATE,
	IN `end_date` DATE,
	IN `project_id` INT,
	IN `username` VARCHAR(50),
...
BEGIN
	...
	SET myquery = CONCAT('SELECT t.finishedBy AS ''t.finishedBy'', COUNT(t.id) AS So_luong_task_hoan_thanh
							   FROM (SELECT *,
								...
								LEFT JOIN zt_dept d
								ON u.dept = d.id
								LEFT JOIN zt_project p
								ON p.id = t.project
								WHERE 1=1');
	SET myquery = CONCAT(myquery, ' AND t.deleted = ''0''');	
	SET myquery = CONCAT(myquery, ' AND u.deleted = ''0'' AND p.deleted = ''0''');	
	...
	CASE
		-- col, val binh thuong
		WHEN col IS NOT NULL AND col <> 's.title' AND col <> 'p.name' AND col <> 't.pri'THEN SET
			myquery = CONCAT(myquery,' AND (', col, ' LIKE N\'%', val ,'%\')');
		
		-- Loc theo story
		WHEN col IS NOT NULL AND col = 's.title' THEN SET
			myquery = CONCAT(myquery,' AND t.story IN (SELECT id FROM zt_story WHERE title=N\'',val,'\' AND deleted <> ''1'')' );
			
		-- Loc theo du an
		WHEN col IS NOT NULL AND col = 'p.name' THEN SET
			myquery = CONCAT(myquery,' AND t.project = (SELECT id FROM zt_project WHERE name=N\'',val,'\' AND deleted <> ''1'')' );
			
		-- Loc theo uu tien
		WHEN col IS NOT NULL AND col = 't.pri' THEN SET
			myquery = CONCAT(myquery,' AND t.pri = CONVERT(RIGHT(\'',val,'\',1),SIGNED INTEGER)' );
...
-- Dumping structure for procedure fpms.dsa_task_taskopened_bar
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `dsa_task_taskopened_bar`(
	IN `start_date` DATE,
	IN `end_date` DATE,
	IN `project_id` INT,
	IN `username` VARCHAR(50),
...
BEGIN
	...
	/* Hiển thị account NV, tên NV và tổng số dự án hoàn thành */
	SET myquery = CONCAT('SELECT t.openedBy AS ''t.openedBy'', COUNT(t.id) AS So_luong_task_hoan_thanh
							   FROM (SELECT *,
									   (CASE WHEN DATE(finishedDate) != ''0000-00-00'' AND deadline = ''0000-00-00'' THEN ''Không có deadline''
												WHEN DATE(finishedDate) != ''0000-00-00'' AND deadline != ''0000-00-00'' AND (DATE(finishedDate) > deadline 
																													OR (DATE(finishedDate) = deadline AND (consumed > estimate))) THEN ''Nộp trễ''
...
BEGIN
	...
	SET QUERY1 = CONCAT(QUERY1, 'AND DATEDIFF(DATE(NOW()), DATE(t.deadline)) >= 7 AND t.STATUS IN (''doing'', ''wait'', ''pause'')AND NOT (t.deadline = ''0000-00-00'' AND t.status=''wait'') AND t.deleted <> ''1'' AND p.deleted <> ''1'' AND t.parent<>-1 ');
...
BEGIN
   ...
   /*Overview*/
	...
	IF id_proc = 'bar_project_task' THEN
		CALL dsa_user_project_task_bar (start_date,end_date,project_id,username,dept_id,col,val);
	END IF;
	...
	IF id_proc = 'line_taskopened' THEN
		CALL dsa_user_taskopened_bytime_line (start_date,end_date,project_id,username,dept_id,col,val,year_,month_,day_,quarter_);
	END IF;
	
	IF id_proc = 'line_taskdone' THEN
		CALL dsa_user_taskdone_bytime_line (start_date,end_date,project_id,username,dept_id,col,val,year_,month_,day_,quarter_);
	END IF;
	...
	IF id_proc = 'user_0_login' THEN
		CALL dsa_user_without_login (start_date,end_date,project_id,username,dept_id,col,val);
	END IF;
	
	IF id_proc = 'table_project_task' THEN
		CALL dsa_user_project_task_table(start_date,end_date,project_id,username,dept_id,col,val);
	END IF;
END//
DELIMITER ;
...
BEGIN
	...
	/* Hiển thị account NV, tên NV và tổng số dự án hoàn thành */
	SET myquery = CONCAT('SELECT tm.account,COUNT(DISTINCT tm.id)
								FROM zt_team tm
								JOIN zt_project p
								ON tm.root = p.id
								JOIN zt_user u
...
-- Dumping structure for procedure fpms.dsa_user_project_task_bar
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `dsa_user_project_task_bar`(
	IN `start_date` DATE,
	IN `end_date` DATE,
	IN `project_id` INT,
	IN `username` VARCHAR(50),
...
BEGIN
	...
	/* Hiển thị account NV, tên NV và tổng số dự án hoàn thành */
	SET myquery = CONCAT('SELECT DISTINCT u.account AS ''account_nv'', p.status AS ''status_project'' , COUNT(DISTINCT p.id) AS ''num_project''
							   FROM (SELECT *,
									   (CASE WHEN DATE(finishedDate) != ''0000-00-00'' AND deadline = ''0000-00-00'' THEN ''Không có deadline''
												WHEN DATE(finishedDate) != ''0000-00-00'' AND deadline != ''0000-00-00'' AND (DATE(finishedDate) > deadline 
																													OR (DATE(finishedDate) = deadline AND (consumed > estimate))) THEN ''Nộp trễ''
	...
	SET myquery = CONCAT(myquery, ' GROUP BY 1,2 ORDER BY 1');
...
-- Dumping structure for procedure fpms.dsa_user_project_task_table
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `dsa_user_project_task_table`(
	IN `start_date` DATE,
	IN `end_date` DATE,
	IN `project_id` INT,
	IN `username` VARCHAR(50),
...
BEGIN
	...
	/* Hiển thị account NV, tên NV và tổng số dự án hoàn thành */
	SET myquery = CONCAT('SELECT DISTINCT u.account AS NV_Account, GROUP_CONCAT(DISTINCT CONCAT(p.name,'' ('',p.status,'')'') ORDER BY p.name SEPARATOR ''\n'' )
							   FROM (SELECT *,
									   (CASE WHEN DATE(finishedDate) != ''0000-00-00'' AND deadline = ''0000-00-00'' THEN ''Không có deadline''
												WHEN DATE(finishedDate) != ''0000-00-00'' AND deadline != ''0000-00-00'' AND (DATE(finishedDate) > deadline 
																													OR (DATE(finishedDate) = deadline AND (consumed > estimate))) THEN ''Nộp trễ''
	...
	SET myquery = CONCAT(myquery, ' GROUP BY t.openedBy ORDER BY t.openedBy');
...
BEGIN
	...
	SET QUERY5 = CONCAT('SELECT CONCAT(YEAR(t.finishedDate),'' - Q'', QUARTER(t.finishedDate)) AS thoi_gian, COUNT(t.id) AS SL_task_hoan_thanh
							   FROM (SELECT *,
									   (CASE WHEN DATE(finishedDate) != ''0000-00-00'' AND deadline = ''0000-00-00'' THEN ''Không có deadline''
												WHEN DATE(finishedDate) != ''0000-00-00'' AND deadline != ''0000-00-00'' AND (DATE(finishedDate) > deadline 
																													OR (DATE(finishedDate) = deadline AND (consumed > estimate))) THEN ''Nộp trễ''
	...
	IF (username IS NOT NULL) THEN
		...
		SET QUERY3 = CONCAT(QUERY3,' AND t.finishedBy = ''', username, '''');
		SET QUERY4 = CONCAT(QUERY4,' AND t.finishedBy = ''', username, '''');	
		SET QUERY5 = CONCAT(QUERY5,' AND t.finishedBy = ''', username, '''');									
   END IF;										
	...
	IF (start_date IS NOT NULL) AND (end_date IS NOT NULL) THEN
		...
		SET QUERY5 = CONCAT(QUERY5,' AND DATE(t.finishedDate) BETWEEN ''', start_date, ''' AND ''',  end_date, ''' '); 
	END IF;	
	...
	IF (project_id IS NOT NULL) THEN
		...
		SET QUERY3 = CONCAT(QUERY3,' AND t.project = ''', project_id, '''');
		SET QUERY4 = CONCAT(QUERY4,' AND t.project = ''', project_id, '''');
		SET QUERY5 = CONCAT(QUERY5,' AND t.project = ''', project_id, '''');
	END IF;
	
	/*Lọc SL task theo department*/
	IF (dept_id IS NOT NULL) THEN
		SET QUERY1 = CONCAT(QUERY1,' AND u.dept = ''', dept_id, '''');
		SET QUERY2 = CONCAT(QUERY2,' AND u.dept = ''', dept_id, '''');
		SET QUERY3 = CONCAT(QUERY3,' AND u.dept = ''', dept_id, '''');
		SET QUERY4 = CONCAT(QUERY4,' AND u.dept = ''', dept_id, '''');
		SET QUERY5 = CONCAT(QUERY5,' AND u.dept = ''', dept_id, '''');
	END IF;
	...
	IF (col IS NOT NULL) THEN
		...
		SET QUERY5 = CONCAT(QUERY5,' AND ',col, ' = ''', val,''' ');
	END IF;	
	...
	SET QUERY3 = CONCAT(QUERY3,' GROUP BY 1 ORDER BY YEAR(t.finishedDate), MONTH(t.finishedDate)');
	SET QUERY4 = CONCAT(QUERY4,' GROUP BY 1 ORDER BY YEAR(t.finishedDate), MONTH(t.finishedDate), DAY(t.finishedDate)');
	SET QUERY5 = CONCAT(QUERY5,' GROUP BY 1 ORDER BY YEAR(t.finishedDate), QUARTER(t.finishedDate)');
	/*Hiển thị tất cả*/
	IF (year_ IS NULL) AND (month_ IS NULL) AND (day_ IS NULL) AND (quarter_ IS NULL) THEN
...
BEGIN
	...
	SET QUERY4 = CONCAT('SELECT CONCAT(Day(t.openedDate),''/'',Month(t.openedDate),''/'', Year(t.openedDate)) AS thoi_gian, COUNT(t.id) AS SL_task_da_tao
							   FROM (SELECT *,
								...
								ON u.dept = d.id
								WHERE t.deleted = ''0'' AND u.deleted = ''0'' AND t.parent !=-1 ');
	SET QUERY5 = CONCAT('SELECT CONCAT(YEAR(t.openedDate),'' - Q'', QUARTER(t.openedDate)) AS thoi_gian, COUNT(t.id) AS SL_task_da_tao
							   FROM (SELECT *,
									   (CASE WHEN DATE(finishedDate) != ''0000-00-00'' AND deadline = ''0000-00-00'' THEN ''Không có deadline''
												WHEN DATE(finishedDate) != ''0000-00-00'' AND deadline != ''0000-00-00'' AND (DATE(finishedDate) > deadline 
																													OR (DATE(finishedDate) = deadline AND (consumed > estimate))) THEN ''Nộp trễ''
	...
	IF (username IS NOT NULL) THEN
		...
		SET QUERY3 = CONCAT(QUERY3,' AND t.openedBy = ''', username, '''');
		SET QUERY4 = CONCAT(QUERY4,' AND t.openedBy = ''', username, '''');	
		SET QUERY5 = CONCAT(QUERY5,' AND t.openedBy = ''', username, '''');										
   END IF;										
	...
	IF (start_date IS NOT NULL) AND (end_date IS NOT NULL) THEN
		...
		SET QUERY5 = CONCAT(QUERY5,' AND DATE(t.openedDate) BETWEEN ''', start_date, ''' AND ''',  end_date, ''' '); 
	END IF;	
	...
	SET QUERY3 = CONCAT(QUERY3,' GROUP BY 1 ORDER BY YEAR(t.openedDate), MONTH(t.openedDate)');
	SET QUERY4 = CONCAT(QUERY4,' GROUP BY 1 ORDER BY YEAR(t.openedDate), MONTH(t.openedDate), DAY(t.openedDate)');
	SET QUERY5 = CONCAT(QUERY5,' GROUP BY 1 ORDER BY YEAR(t.openedDate), QUARTER(t.openedDate)');
...
BEGIN
	...
	/*Lọc theo khoảng thời gian*/
	IF (start_date IS NOT NULL) AND (end_date IS NOT NULL) THEN
		SET myquery  = CONCAT(myquery ,' AND DATE(a.date) BETWEEN ''', start_date, ''' AND ''',  end_date, ''' '); 
	END IF;	
	...
	/*Lọc SL task theo department*/
	IF (dept_id IS NOT NULL) THEN
		SET myquery  = CONCAT(myquery,' AND dept = ''', dept_id, '''');
	END IF;
	...
	SET myquery  = CONCAT(myquery , ' GROUP BY u.account ORDER BY COUNT(t.id) DESC, u.account');
...
-- Dumping structure for procedure fpms.dsa_user_without_login
DELIMITER //
...
BEGIN
	...
	SET myquery  = CONCAT('SELECT u.account 
								...
								WHERE 1=1 ');
	SET myquery  = CONCAT(myquery , ' AND u.deleted = ''0''');	
	SET myquery  = CONCAT(myquery , ' AND a.objectType = ''user''');
	SET myquery  = CONCAT(myquery , ' AND a.action = ''login'' ');
		/*Lọc theo khoảng thời gian*/
	...
	SET myquery1 = CONCAT ('SELECT u.account
									FROM zt_user u
									LEFT JOIN zt_dept d
									ON dept = d.id
...
BEGIN
	...
	END IF;
		/*Lọc theo khoảng thời gian*/
	IF (start_date IS NOT NULL) AND (end_date IS NOT NULL) THEN
		SET myquery1  = CONCAT(myquery1,' AND DATE(a.date) BETWEEN ''', start_date, ''' AND ''',  end_date, ''' '); 
	END IF;	
...
-- Dumping structure for procedure fpms.dsa_user_without_task_
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `dsa_user_without_task_`(
	IN `start_date` DATE,
	IN `end_date` DATE,
	IN `project_id` INT,
	IN `username` VARCHAR(50),
