...
def gen_layout():
    ...
    layout =html.Div([
                        html.Div([
                            html.Div([
                                html.Div("CHI TIẾT PROJECT THEO NHÂN VIÊN", style={'width':'100%','fontWeight': 'bold','background': '#FFD45D'}),
                                html.A(
                                    children=[
                                        html.I(className="fas fa-file-download")
                                    ],
                                    id='download_detail_project',
                                    download="DETAIL_USERS_PROJECT.csv",
                                    target="_blank",
                                    href = '',
...
@app_User.callback(
    Output("detail_PROJECT", "data"),
    Output("detail_PROJECT", "style_data_conditional"),
...
def UPDATE_DETAIL_PROJECT(start_date,end_date,ddl_project,ddl_user,ddl_dept,user_opened,user_done,user_project,user_started,user_project_task,ac_project,ac_user,ac_log,ac_project_task,ac_workload,data_project, data_user,data_log,data_project_task,data_workload):
    ...
    df =DB_User.GET_USER(('table_project',start_date,end_date,ddl_project,ddl_user,ddl_dept,label,value,None,None,None,None))
...
@app_User.callback(
     Output('download_detail_project', 'href'),
    [Input('download_detail_project','n_clicks'),
     Input('detail_PROJECT','data')]
)
