...
def UPDATE_comment_detail(date_type, start_date, end_date, potential, sales_stage, product, dept, assign, page_current, page_size, sort_by, filter):
    ctx = dash.callback_context
    datatable = {}
    label, value = GParams_Potential.Get_Value(ctx, datatable)

    df = DB_potential.Get_potential(('comment_detail', date_type, start_date, end_date, None, None, None, None,
...
def UPDATE_DOWNLOAD(click, date_type, start_date, end_date, potential, sales_stage, product, dept, assign):
    ...
    if df.empty:
        df = pd.DataFrame({'company': ['Không có giá trị'], 'u.user_name': [
                          'Không có giá trị'], 'comment': [0], 'answer': [0]})
        csv_string = df.to_csv(index=False, encoding='utf-8',
                               header=['Công ty', 'Nhân viên được giao', 'Bình luận', 'Trả lời'])
    else:
        csv_string = df.to_csv(index=False, encoding='utf-8',
                               header=['Công ty', 'Nhân viên được giao', 'Bình luận', 'Trả lời'])
