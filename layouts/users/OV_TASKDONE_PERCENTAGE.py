...
def gen_layout():
    ...
    title = 'ĐỘ HOÀN THÀNH CỦA TASK ĐÃ TẠO (%)'
    color = '#FFD45D'
    icon = 'ov_percentage.svg'
    layout = create_card(title,'ov-taskdone-percentage',color,icon)
           
    return layout

@app_User.callback(
    Output("ov-taskdone-percentage","children"),
...
def UPDATE_OV_TASKDONE(start_date,end_date,ddl_project,ddl_user,ddl_dept,user_opened,user_done,taskdone_status,user_project,user_started,user_project_task,ac_project,ac_user,ac_log,ac_project_task,ac_workload,data_project, data_user,data_log,data_project_task,data_workload):
    ...
    df = DB_User.GET_USER(('ov_taskdone_percentage',start_date,end_date,ddl_project,ddl_user,ddl_dept,label,value,None,None,None,None))


    
    return df['Độ hoàn thành của task đã tạo (%)'].values[0].decode("utf-8")
