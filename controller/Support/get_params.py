...
class Get_Params:
    ...
    def Get_Value_DT(self, active_cell, data, cols):
        ...
        if label in cols.keys() and label in ['c2.label', 'company']:
            print(label)
            print(data[active_cell['row']][cols[label]])
            value = int(data[active_cell['row']][cols[label]].split('&record=')[1][:-1])
            print(value)
            return label, value

        elif label in cols.keys() and label not in ['c2.label', 'company']:
