...
def gen_layout():
    ...
    title = 'Tổng hỗ trợ đang mở'
    color = 'linear-gradient(to right, rgb(34, 193, 195), rgb(253, 187, 45))'
    icon = 'close.svg'

    layout = create_card(title, 'ov_total_open_potential', color, icon)

    return layout


@app_Support.callback(
    Output("ov_total_open_potential", "children"),
...
def update_ov_total_support(date_type, start_date, end_date, account, account_status, user, dept, pareto_user, bar_status, pie_pri, pie_location, ac_delay, ac_detail, data_delay, data_detail):
    ...
    values = DB_Support.get_Support(('ov_support_open', date_type, start_date,
                                    end_date, account, account_status, user, dept, None, None, label, value))
    total = DB_Support.get_Support(('ov_support', date_type, start_date,
                                   end_date, account, account_status, user, dept, None, None, label, value))
