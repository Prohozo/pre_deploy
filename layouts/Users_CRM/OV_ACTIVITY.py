...
def gen_layout():
    ...
    color = 'linear-gradient(50deg, rgba(255,209,67,1) 0%, rgba(255,141,85,1) 100%)'
    icon = 'ov_activity.svg'
    layout = create_card(title,'ov_activity',color,icon)
           
    return layout

@app_UsersCRM.callback(
    Output("ov_activity","children"),
    Output("ov_activity_diff","children"),
...
def UPDATE_OV_ACTIVITY(date_value,start_date,end_date,ddl_account,ddl_user_assign,ddl_dept,salesorder,task_created,task_finished,ac_withoutactivity,ac_task_delay,ac_activity_detail,ac_without_login,data_withoutactivity,data_task_delay,data_activity_detail,data_without_login):
    ...
    df = DB_Users_CRM.GET_USERSCRM(('ov_activity',start_date,end_date,ddl_account,ddl_user_assign,ddl_dept,label,value))
    
    if date_value is not None:
        start_date_last,end_date_last = last_time(date_value)
        df2 = DB_Users_CRM.GET_USERSCRM(('ov_activity',start_date_last,end_date_last,ddl_account,ddl_user_assign,ddl_dept,label,value))

        end_value = df['Tổng số lượng hoạt động'].values[0]
        start_value  = df2['Tổng số lượng hoạt động'].values[0]
        ...
        return df['Tổng số lượng hoạt động'].values[0], '-'
