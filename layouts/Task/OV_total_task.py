...
def gen_layout():
    ...
    title = 'Tổng số task đã tạo'
    color = '#60B664'
    icon = 'task.svg'

    layout = create_card(title,'ov_total_task',color, icon)
    
    return layout

@app_Task.callback(
    Output("ov_total_task", "children"),
    Output("ov_total_task_diff", "children"),
...
def update_ov_total_task(date_value, start_date, end_date, project_id, user, story, dept, ac_task_delay, ac_task_byproject, ac_task_bystory, ac_task_numpri, ac_dept, data_task_delay, data_task_byproject, data_task_bystory):
    ...
    values = DB_Task.Get_task(('total_task', start_date, end_date, project_id, user, story, dept, label, value, None, None, None, None))
    if date_value is not None:
        start_date_2, end_date_2 = last_time(date_value)
        last_values = DB_Task.Get_task(('total_task', start_date_2, end_date_2, project_id, user, story, dept, label, value, None, None, None, None))

        value1 = int(values.total_task)
        value2 = int(last_values.total_task)
