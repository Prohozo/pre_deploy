...
def gen_layout():
    ...
    layout = create_card(title, 'ov_potential', color, icon)

    return layout


@app_Accounts.callback(
    Output("ov_potential", "children"),
    Output("ov_potential_diff", "children"),
...
def UPDATE_OV_POTENTIAL(date_value, ddl_timetype, start_date, end_date, ddl_leadsource, ddl_industry, ddl_account, ddl_user_assign, ddl_dept, status, industry, user_assign, user_marketing, user_service, map_city, ac_user_detail, ac_account_detail, ac_city, data_user_detail, data_account_detail, data_city):
    ...
    df = DB_Accounts.GET_ACCOUNTS(('ov_potential', ddl_timetype, start_date, end_date, ddl_leadsource, ddl_industry,
    ...
    if date_value is not None:
        ...
        df2 = DB_Accounts.GET_ACCOUNTS(('ov_potential', ddl_timetype, start_date_last, end_date_last, ddl_leadsource, ddl_industry,
                                       ddl_account, ddl_user_assign,  None, None,  ddl_dept, None, None, None, None, label, value))

        end_value = df['Tổng số lượng cơ hội'].values[0]
        start_value = df2['Tổng số lượng cơ hội'].values[0]
        ...
        return df['Tổng số lượng cơ hội'].values[0], '-'
