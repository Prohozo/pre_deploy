...
def gen_layout():
    ...
    layout = html.Div([
                html.Div([
                    dcc.Graph(id="grh_bar_taskstarted", style={'width': '100%', 'height': '100%'})
...
@app_User.callback(
    Output("grh_bar_taskstarted","figure"),
...
def UPDATE_SL_TASKOPENED(start_date,end_date,ddl_project,ddl_user,ddl_dept,user_opened,user_done,taskdone_status,user_project,user_started,user_project_task,ac_project,ac_user,ac_log,ac_project_task,ac_workload,data_project, data_user,data_log,data_project_task,data_workload):
    ...
    df = DB_User.GET_USER(('bar_taskstarted',start_date,end_date,ddl_project,ddl_user,ddl_dept,label,value,None,None,None,None))
    return Graph.grh_BarChart(df['account_nv'],df['Số lượng Task'],marker_color=['#FF938B','#FF938B'],title='SỐ LƯỢNG TASK ĐÃ START THEO NHÂN VIÊN',margin = [50, 150, 35, 35])
