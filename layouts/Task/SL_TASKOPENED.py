...
def gen_layout():
    ...
    layout = html.Div([
                html.Div([
                    html.Div([
                                html.B(html.Div("SỐ LƯỢNG TASK ĐÃ TẠO THEO NHÂN VIÊN", style={'width': '100%', 'background': '#60B664', 'color': 'white', 'text-align': 'center'}))
                            ], style={'height': '5%', 'width': '100%', 'background': '#60B664', 'textAlign': 'center',  'alignItems': 'center', 'position': 'relative'}),
                    dcc.Graph(id="bar_taskopened", style={'width': '100%', 'height': '100%'})
...
def update_task_done_bar(start_date, end_date, project_id, user, story, dept, ac_task_delay, ac_task_byproject, ac_task_bystory,ac_task_numpri,ac_dept, data_task_delay, data_task_byproject, data_task_bystory):
    ...
    df = DB_Task.Get_task(('bar_taskopened', start_date, end_date, project_id, user, story, dept, label, value, None, None, None, None))
    
    return Graph_Task.grh_BarChart(df['t.openedBy'],df['So_luong_task_hoan_thanh'],marker_color=['#107a8b','#107a8b'],margin = [35, 150, 35, 35])
